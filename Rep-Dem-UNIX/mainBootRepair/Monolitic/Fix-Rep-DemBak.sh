...
detect_distribution() {
    ...
    if [[ -f /etc/os-release ]]; then
        ...
        # Résolution via ID direct + ID_LIKE pour les dérivées
        local _id_lower="${ID,,}"
        local _id_like_lower="${ID_LIKE,,}"
        ...
        _resolve_distro_family() {
            ...
            slackware*)
                DISTRO_FAMILY="slackware"; PKG_MANAGER="pkgtool" ;;
            *) return 1 ;;
            esac
            return 0
        }
        ...
        if ! _resolve_distro_family "$_id_lower"; then
            ...
            if [[ "$_resolved_via_like" == false ]]; then
                ...
                elif command_exists apk;   then PKG_MANAGER="apk";    DISTRO_FAMILY="alpine"
                else PKG_MANAGER="unknown"
                fi
                log_warning "Distribution inconnue : ${ID} (ID_LIKE=${ID_LIKE:-—}). Famille déduite : $DISTRO_FAMILY"
            fi
        fi
        ...
        if [[ -f /etc/debian_version ]]; then
            ...
            DISTRO="alpine"; DISTRO_FAMILY="alpine"; PKG_MANAGER="apk"
            DISTRO_VERSION=$(cat /etc/alpine-release)
            log_success "Détecté via /etc/alpine-release : Alpine $DISTRO_VERSION"
        elif [[ -f /etc/gentoo-release ]]; then
            DISTRO="gentoo"; DISTRO_FAMILY="gentoo"; PKG_MANAGER="emerge"
            log_success "Détecté via /etc/gentoo-release : Gentoo"
        elif [[ -f /etc/void-release ]]; then
            DISTRO="void"; DISTRO_FAMILY="void"; PKG_MANAGER="xbps"
            log_success "Détecté via /etc/void-release : Void Linux"
        elif command_exists apt;     then DISTRO="unknown"; DISTRO_FAMILY="debian"; PKG_MANAGER="apt"
            log_warning "Distribution inconnue — famille déduite via apt"
        elif command_exists dnf;     then DISTRO="unknown"; DISTRO_FAMILY="rhel"; PKG_MANAGER="dnf"
            log_warning "Distribution inconnue — famille déduite via dnf"
        elif command_exists yum;     then DISTRO="unknown"; DISTRO_FAMILY="rhel"; PKG_MANAGER="yum"
            log_warning "Distribution inconnue — famille déduite via yum"
        elif command_exists pacman;  then DISTRO="unknown"; DISTRO_FAMILY="arch"; PKG_MANAGER="pacman"
            log_warning "Distribution inconnue — famille déduite via pacman"
        elif command_exists zypper;  then DISTRO="unknown"; DISTRO_FAMILY="suse"; PKG_MANAGER="zypper"
            log_warning "Distribution inconnue — famille déduite via zypper"
        elif command_exists apk;     then DISTRO="unknown"; DISTRO_FAMILY="alpine"; PKG_MANAGER="apk"
